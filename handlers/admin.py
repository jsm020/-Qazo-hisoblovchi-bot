from database.db import (
    get_stats, add_channel, get_channels, remove_channel, add_admin, remove_admin, get_admins, ensure_main_admin,
    add_faq, create_faq_table, get_all_user_ids,
    add_notify_time, remove_notify_time, get_notify_times
)
...
async def stats_handler(message: Message):
    if not await is_admin(message.from_user.id):
        await message.answer("Bu amalni faqat administratorlar bajara oladi.")
        return
...
async def broadcast_content(message: Message, state: FSMContext):
    ...
    await message.answer(
        "Xabar quyidagicha ko‘rinadi:\n\n(Tasdiqlash uchun '✅ Yuborish', bekor qilish uchun '❌ Bekor qilish' deb yozing)",
        reply_markup=markup)
    # Preview
    if message.text:
...
@router.message(F.text.in_(["/addtime", "addtime"]))
...
async def process_add_time(message: Message, state: FSMContext):
    try:
        ...
        await message.answer("Iltimos, vaqtni HH:MM formatda kiriting (masalan, 17:04).")
    await state.clear()

@router.message(F.text.in_(["/removetime", "removetime"]))
...
@router.message(F.text.in_(["/times", "times"]))
async def list_times_handler(message: Message):
    ...
    if not times:
        await message.answer("Hozircha belgilangan vaqtlar mavjud emas.")
    else:
